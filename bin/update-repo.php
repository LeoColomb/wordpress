...
{
  $wpReleasesPath = getcwd() . '/cache/wp-releases.json';
...
function tempdir()
{
  $tempfile = tempnam(sys_get_temp_dir(), 'leocolomb-wp-');
...
function getGitRemote() {
  ...
  $githubRepo = getenv('TRAVIS_REPO_SLUG');
...
function configureGitRepo() {
  ...
  return (
    run("git config user.email $githubUser")
  );
}
...
function validateRelease($release) {
  ...
  curl_close($ch);
  if ($statusCode !== 200) {
    throw new \RuntimeException("tag '$version' does does not have a reachable dist url (status $statusCode for {$release->distball_url})");
  }
